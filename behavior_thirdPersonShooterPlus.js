...
// Modified by Ishmaru to have better Animation Priorities, Mouse Jumping Option, and Left And Right button Actions Enabled
//
// The following embedded xml is for the editor and describes how the behavior can be edited:
// Supported types are: int, float, string, bool, color, vect3d, scenenode, texture, action
/*
	<behavior jsname="behavior_thirdPersonShooterPlus" description="Third person shooter player plus">
		...
		<property name="WalkBackAnimation" type="string" default="walk" />
		<property name="JumpAnimation" type="string" default="jump" />
		<property name="LeftAnimation" type="string" default="shoot" />
		<property name="RightAnimation" type="string" default="shoot1" />
		<property name="DeathAnimation" type="string" default="die" />
		<property name="MouseJump" type="bool" default="false" />		
		<property name="StopOnAction" type="bool" default="false" />
		<property name="LeftMouseAction" type="action"/>
		<property name="RightMouseAction" type="action"/>
...
behavior_thirdPersonShooterPlus = function()
{
...
behavior_thirdPersonShooterPlus.prototype.doCameraMovement = function(camera, node, timeMs)
{
...
behavior_thirdPersonShooterPlus.prototype.dotProduct = function(a, b)
{
	return a.x*b.x + a.y*b.y + a.z*b.z;
}

behavior_thirdPersonShooterPlus.prototype.getPlaneIntersection = function(planey, linepoint, linevect)
{
...
behavior_thirdPersonShooterPlus.prototype.onAnimate = function(currentnode, timeMs)
{
	name = ccbGetSceneNodeProperty(currentnode, 'Name');
	// print(name)
	// health = ccbGetCopperCubeVariable("#player.health")
	...
	if (mouseposonground != null  && health > 0)
	{
	...
	*/
	...
	if ((bForward || bBackward)  && health > 0) // || (this.UseAcceleration && this.AcceleratedSpeed != 0))
	{
	...
	if ((bForward) && health > 0 )
	{
		ccbSetSceneNodeProperty(node, 'Animation', this.WalkAnimation);
		ccbSetSceneNodeProperty(node, 'Looping', true);
	}
	else
	if ((bBackward) && health > 0 )
	{
		if (this.WalkBackAnimation)
		{
			ccbSetSceneNodeProperty(node, 'Animation', this.WalkBackAnimation);
			ccbSetSceneNodeProperty(node, 'Looping', true);
		}
		else
		if (!this.WalkBackAnimation)
	...
	if (this.LeftMouse == true && health > 0)
	{
		ccbSetSceneNodeProperty(node, 'Animation', this.LeftAnimation);
		ccbSetSceneNodeProperty(node, 'Looping', false);
	}
	else
	if (this.RightMouse == true && health > 0)
	{
		ccbSetSceneNodeProperty(node, 'Animation', this.RightAnimation);
		ccbSetSceneNodeProperty(node, 'Looping', false);
	}
	else
	if (health <= 0 )
	...
	if (this.PressedJump && this.JumpForce == 0  && health > 0)
	{
		this.PressedJump = false;
...
behavior_thirdPersonShooterPlus.prototype.onKeyEvent = function(code, down)
{
...
behavior_thirdPersonShooterPlus.prototype.onMouseEvent = function(mouseEvent, mouseWheelDelta)
{
